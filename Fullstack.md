# Finmid Frontend Fullstack Challenge
...
As well, we ask to extend the API with new methods.
...
Feel free to use any React framework of your choice.
Use any `npm` libraries you need.

I you want to rewrite the app using Next, Nest, Deno, or anything else &mdash; we'd be happy to check it out!
...
3. Display the transaction feed.
  - Display only current SME's transactions
  - Users should be able to filter by status of the transaction except for `REVERSED`
  - Each transaction item must display:
    - Merchant name
    - Merchant icon
    - Formatted date (`dd.mm.yyyy HH:mm` format)
...
4. Extend the API of transactions controller
  - Add APIs to create, delete, and update the transactions
  - Validate the requests using your best-guess assumptions
  - No UI interactions is needed for this part
5. Bonus task. There's a security vulnerability in the API, can you identity what it is?
